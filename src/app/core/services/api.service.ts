...
export class ApiService {
    ...
    getUserWorkspaces(): Observable<WorkspaceListModel> {
        return this.http.get<WorkspaceListModel>(this.settings.apiUrl + '/v1/workspaces');
    }
    ...
    getWorkspace(workspace_id: string, policies = false, groups = false, members = false, polls = false): Observable<WorkspaceModel> {
        ...
        return this.http.get<WorkspaceModel>(this.settings.apiUrl + '/v1/workspaces/' + workspace_id, { params: params });
    }
    ...
    deleteWorkspace(workspace_id: string) {
        return this.http.delete(this.settings.apiUrl + '/v1/workspaces/' + workspace_id);
    }

    // Get list of members in workspace
    getWorkspaceMembers(workspace_id: string): Observable<MemberListModel> {
        return this.http.get<MemberListModel>(this.settings.apiUrl + '/v1/workspaces/' + workspace_id + '/members');
    }

    // Add member to workspace
    addMemberToWorkspace(workspace_id: string, data: any) {
        return this.http.post(this.settings.apiUrl + '/v1/workspaces/' + workspace_id + '/members', data);
    }

    // Remove member from workspace
    removeMemberFromWorkspace(workspace_id: string, member_workspace_id: string) {
        return this.http.delete(this.settings.apiUrl + '/v1/workspaces/' + workspace_id + '/members/' + member_workspace_id);
    }
    ...
    getWorkspacePolicies(workspace_id: string, account_id?: string): Observable<PolicyListModel> {
        ...
        return this.http.get<PolicyListModel>(this.settings.apiUrl + '/v1/workspaces/' + workspace_id + '/policies', options);
    }

    // Update workspace policy
    updateWorkspacePolicy(workspace_id: string, policy_id: string, data: any) {
        return this.http.put(this.settings.apiUrl + '/v1/workspaces/' + workspace_id + '/policies/' + policy_id, data);
    }
    ...
    getWorkspaceGroups(id: string): Observable<GroupListModel> {
        return this.http.get<GroupListModel>(this.settings.apiUrl + '/v1/workspaces/' + id + '/groups');
    }
    ...
    getAllPolls(workspace_id: string): Observable<PollListModel> {
        return this.http.get<PollListModel>(this.settings.apiUrl + '/v1/workspaces/' + workspace_id + '/polls');
    }

    // Create poll in workspace
    createPoll(workspace_id: string, data: NewPollRequestBody): Observable<PollModel> {
        return this.http.post<PollModel>(this.settings.apiUrl + '/v1/workspaces/' + workspace_id + '/polls', data);
    }
